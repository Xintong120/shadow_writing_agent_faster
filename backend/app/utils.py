    ...
    def get_client(self) -> httpx.Client:
        ...
        if self._client is None:
            self._client = httpx.Client(
                ...
                limits=httpx.Limits(
                    max_keepalive_connections=10,  # 保持连接的最大数量
                    max_connections=20,             # 总连接池大小
                    keepalive_expiry=30.0          # 保持连接的过期时间
                ),
                # SSL 配置
...
def get_http_client() -> httpx.Client:
    ...
    Returns:
        httpx.Client: 共享的 HTTP 客户端实例
    """
    return http_client_manager.get_client()


def initialize_litellm_client():
    """初始化 LiteLLM 的全局 HTTP 客户端

    设置全局 HTTP 客户端以实现连接池复用，减少 SSL 握手开销
    """
    import litellm

    # 设置全局同步 HTTP 客户端
    litellm.client_session = get_http_client()

    print("LiteLLM 全局 HTTP 客户端已设置（连接池复用已启用）")
